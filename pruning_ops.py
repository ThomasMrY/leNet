...
def pruning(scale):
	...
	for x in tf.trainable_variables():
		if ('convolutional_layer' in x.op.name or 'full_connected_layer' in x.op.name):
			zeros = tf.zeros_like(x)
			ones = tf.ones_like(x)
			sigma = tf.reduce_max(tf.abs(x))*scale
			try:
				with tf.variable_scope('mask', reuse=Ture) as scope:
					mask = tf.get_variable(x.op.name,shape = x.shape)
			except:
				with tf.variable_scope('mask', reuse=False) as scope:
					mask = tf.get_variable(x.op.name,shape = x.shape)
