...
layer1 = add_cov_layer(x_imgs,1,[5,5,1,10],activation_function = tf.nn.relu)
...
layer4_b,alpha_2 = add_2_binary(layer4,20)
layer5 = add_full_layer(layer4_b,5,60,alpha_2,activation_function = tf.nn.relu)
y = add_full_layer(layer5,6,10,activation_function =tf.nn.softmax)
#########################################################
...
#########################################################
pruning_sess = pops.pruning(scale = 0.01)
...
with tf.Session(config = tf.ConfigProto(log_device_placement = True)) as sess:
	...
	for i in range(Train_steps):
		...
		if(i%100 == 0):
			accuracy_batch = sess.run(accuracy,feed_dict = {x:batch[0],y_:batch[1]})
			loss_batch = sess.run(cross_entropy,feed_dict = {x:batch[0],y_:batch[1]})
